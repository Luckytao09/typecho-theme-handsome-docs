...
1. 缩小版头图优化样式
2. 加密文章样式优化
3. 全面多语言化主题中的中文，包括所有的提示语句
4. 改进了提示信息泡的样式和动画效果
5. 重写延迟加载部分代码
6. 将部分由js控制样式改为直接由css控制，加载速度更快
7. 重构了主题部分代码，优化细节部分
1. **修正点击首页说说链接未兼容伪静态**
2. **修复了侧边栏在固定+折叠时候无法展开的bug**（这个陈年老bug终于修好了！）
3. 修复了侧边栏的最新评论中如果评论为空导致的样式错误
4. 修复了时光机中没有说说的时候首页输出网址的bug
5. 修复了分类页面标题和首页文章标题统一，只显示一行
6. **修复开启平滑滚动出现的页面错乱问题**
...
2. **增加了ajax评论的更好的体验（实验性功能，需要手动在`后台外观设置—>增强功能开关` 开启）**
3. 增加了增加平滑滚动功能（后续会优化此体验）
4. 增加了左侧和顶部自定义栏目可选择是否新窗口打开（target字段，具体见使用文档更新）
5. 增加使用文档可以全局搜索的功能（ctrl+F5 获取最新版本）
1. 增加音乐解析功能（获取网易云永久链接）
1. 优化了评论区域的超链接样式以及增加评论区域图片的灯箱效果
2. 优化评论区关闭时候的样式
2. 优化了小板式缩略图显示样式
3. 优化了缩略图显示方式
4. 优化手机端标题显示
...
1. 修复了部分语言未翻译
2. 修复了打赏图标的错误
3. **修复了首页名称的显示规则（顶部导航栏显示【首页名称】，头像名称下面显示【博主名称】，右侧名称显示为【博客标题】）**
4. 修复了手机端文章顶部信息样式错误问题
5. 修复了音乐播放器未点击播放加载第一首歌曲资源
6. 修复了部分服务器中 文章时间显示时差问题
7. 修复了缩略图显示bug（当关闭显示头图开关，thumb填写图片地址，不能够显示头图的问题）
8. 修复非伪静态站点搜索框出现404错误
9. 修复分享到QQ空间地址错误问题
